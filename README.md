...
- 新規インストールでは消費速度のローカル計測をデフォルトで有効化
...
4. `Consumption speed tracking`が有効であることを確認する。新規インストールでは有効
5. 必要に応じて、8週間のPace予測履歴を保存する`Historical tracking`を有効にする
6. メニューからCodexを更新する

消費速度の計測は`Consumption speed tracking`だけで動作します。`Historical tracking`は上流版由来のPace予測用履歴などを制御する別設定です。`OpenAI web extras`はコードレビュー枠やWebダッシュボード由来の補助情報にだけ必要で、消費速度の計測には不要です。
...
`Consumption speed tracking`は新規インストールで有効です。既存環境で履歴記録を明示的に無効化していた場合は、その選択を引き継ぎます。独自の外部送信先は追加していません。速度履歴は次のファイルへローカル保存します。
